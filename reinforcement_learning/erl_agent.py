...
class AgentDoubleDQN:
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        ...
        self.act = act_class(net_dims, state_dim, action_dim).to(self.device)
        self.cri = cri_class(net_dims, state_dim, action_dim).to(self.device) \
    ...
    def get_obj_critic(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        with torch.no_grad():
            ...
            if torch.isnan(states).any():
                print(f"Warning: NaN in states")
                return torch.tensor(0.0, device=self.device, requires_grad=True), torch.zeros_like(actions, dtype=torch.float32)
            if torch.isnan(rewards).any():
                print(f"Warning: NaN in rewards")
                return torch.tensor(0.0, device=self.device, requires_grad=True), torch.zeros_like(actions, dtype=torch.float32)
            if torch.isnan(next_ss).any():
                print(f"Warning: NaN in next_ss")
                return torch.tensor(0.0, device=self.device, requires_grad=True), torch.zeros_like(actions, dtype=torch.float32)

            next_qs = torch.min(*self.cri_target.get_q1_q2(next_ss)).max(dim=1, keepdim=True)[0].squeeze(1)
            if torch.isnan(next_qs).any():
                print(f"Warning: NaN in next_qs from target network")
                return torch.tensor(0.0, device=self.device, requires_grad=True), torch.zeros_like(actions, dtype=torch.float32)
                
            q_labels = rewards + undones * self.gamma * next_qs

        q1, q2 = [qs.gather(1, actions.long()).squeeze(1) for qs in self.cri.get_q1_q2(states)]
        
        # 检查数值稳定性
        if torch.isnan(q1).any() or torch.isnan(q2).any() or torch.isnan(q_labels).any():
            print(f"Warning: NaN detected in Q values or labels")
            print(f"q1 has NaN: {torch.isnan(q1).any()}, q2 has NaN: {torch.isnan(q2).any()}")
            print(f"q_labels has NaN: {torch.isnan(q_labels).any()}")
            return torch.tensor(0.0, device=self.device, requires_grad=True), q1
        
        obj_critic = self.criterion(q1, q_labels) + self.criterion(q2, q_labels)
        
        # 检查损失是否为NaN
        if torch.isnan(obj_critic):
            print(f"Warning: NaN loss detected, setting to 0")
            obj_critic = torch.tensor(0.0, device=self.device, requires_grad=True)
    ...
    def update_net(self, buffer: ReplayBuffer) -> Tuple[float, ...]:
        ...
        for _ in range(update_times):
            ...
            obj_actors += 0.0  # DQN没有单独的actor网络，设置为0
